$(document).ready(function() {
 ...
 //var baseURL="";
  var APIKey = "061441a6001eb3a4512c89351b0851ff";
  //var units = "imperial";
  var units="imperial";
...
function colorModePreview(ele) {
    if($(ele).prop("checked") == true){
      ...
      //baseURL = "https://api.openweathermap.org/data/2.5/weather?&units=Celcius";
      units="imperial"

    }
    else if($(ele).prop("checked") == false){
      console.log("Celcius")
      //baseURL = "https://api.openweathermap.org/data/2.5/weather?&units=Farenhiet";
      units="metric"
  ...
  function displayCurrentWeather(data) {
    ...
    $("#wind-speed").text(`${data.current.wind.speed} mph`);
      ...
      $("#city").text(data.current.name);
    $("#conditions").text(data.current.weather[0].main);
    $("#temperature").text(`${parseInt(data.current.main.temp)}\u00B0 C`);
    $("#humidity").text(`${data.current.main.humidity}%`);
    $("#wind-speed").text(`${data.current.wind.speed} kph`);
    }
  ...
  function displayForecast(data) {
    ...
    $.each(forecast, function(i, day) {
      ...
      $(`#day-${i + 1}-humidity`).text(`${day.main.humidity}% Humidity`);
        ...
        $(`#day-${i + 1}-year`).text(year);
        $(`#day-${i + 1}-conditions`).text(day.weather[0].main);
        $(`#day-${i + 1}-temp`).text(`${parseInt(day.main.temp)}\u00B0 C`);
        $(`#day-${i + 1}-humidity`).text(`${day.main.humidity}% Humidity`);
      }
    });
